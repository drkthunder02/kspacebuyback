...
class FetchCorporationStructures implements ShouldQueue
{
    ...
    public function failed($exception) {
        ...
        if ((is_object($exception->getEsiResponse()) && (stristr($exception->getEsiResponse()->error, 'Too many errors') || stristr($exception->getEsiResponse()->error, 'This software has exceeded the error limit for ESI'))) || 
           (is_string($exception->getEsiResponse()) && (stristr($exception->getEsiResponse(), 'Too many errors') || stristr($exception->getEsiResponse(), 'This software has exceeded the error limit for ESI')))) {
            ...
            Log::info('FetchCorporationStructures has hit the error rate limiter.  Releasing the job back into the wild in 2 minutes.');
            $this->release(120);
        }  else {
            ...
            switch($errorCode) {
                case 400:  //Bad Request
                    Log::critical("Bad request has occurred in FetchCorporationStructures.  Job has been discarded");
                    break;
                case 401:  //Unauthorized Request
                    Log::critical("Unauthorized request has occurred in FetchCorporationStructures at " . Carbon::now()->toDateTimeString() . ".\r\nCancelling the job.");
                    break;
                case 403:  //Forbidden
                    Log::critical("FetchCorporationStructures has incurred a forbidden error.  Cancelling the job.");
                    break;
                case 420:  //Error Limited
                    Log::warning("Error rate limit occurred in FetchCorporationStructures.  Restarting job in 120 seconds.");
                    $this->release(120);
                    break;
                case 500:  //Internal Server Error
                    Log::critical("Internal Server Error for ESI in FetchCorporationStructures.  Attempting a restart in 120 seconds.");
                    $this->release(120);
                    break;
                case 503:  //Service Unavailable
                    Log::critical("Service Unavailabe for ESI in FetchCorporationStructures.  Releasing the job back to the queue in 30 seconds.");
                    $this->release(30);
                    break;
                case 504:  //Gateway Timeout
                    Log::critical("Gateway timeout in FetchCorporationStructures.  Releasing the job back to the queue in 30 seconds.");
                    $this->release(30);
                    break;
                ...
                default:
                    Log::warning("No response code received from esi call in FetchCorporationStructures.\r\n");
                    $this->delete();
                    break;
            }
        }
    ...
    public function tags() {
        return ['FetchCorporationStructures', 'AllianceStructures', 'Structures'];
    }
}
